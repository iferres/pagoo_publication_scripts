# setwd("/mnt/4tera/iferres/benchmark_pagoo")
run_roary <- function(gffs, p = 1, f = ".", singularity_image = "containers/roary.sif"){
...
generate_input <- function(gene_presence_absence_csv, gffs, sep = "__"){
	library(Biostrings)
  df <- read.csv(gene_presence_absence_csv, header = TRUE,
  ...
  return(list(data = mm, cluster_meta = cluster_meta, sequences = DNAStringSetList(seqs), sep = sep))
}
...
allgffs <- list.files(path = "decano_downing_2019_dataset_gffs/", pattern = "[.]gff$", full.names = TRUE)
...
timings_dfs <- mcmapply(function(gffs, n, f, singularity_image, p) {

  # gffs <- sample(allgffs, n)
  # roary_out <- run_roary(gffs, p = p, f = f, singularity_image = singularity_image)
...
}, gffs = gffs, n = ns, f = fout, MoreArgs = list(singularity_image = "containers/roary.sif", p = 6), SIMPLIFY = FALSE, mc.cores = 5)
...
library(ggplot2)
g <- ggplot(x, aes(y=Elapsed, x=N, color=Operation, group=N)) + geom_point() + facet_wrap(~Type, scales="free_y", nrow=2) + stat_summary(fun=median, geom="line", aes(group=Operation)) + theme_light() + scale_color_discrete(labels=c("$gg_barplot()", "$core_seqs_4_phylo()", "pagoo()", "roary_2_pagoo()", "roary_2_pagoo() + gff"), guide = guide_legend(reverse=T)) + ylab("Elapsed time (s)") + xlab("Number of organisms")
